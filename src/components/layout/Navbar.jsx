...
export default function Navbar({
  ...
  return (
    <header className="sticky top-0 z-40 w-full px-2 xs:px-4 sm:px-8 py-2.5 sm:py-3.5">
      <div className="max-w-7xl mx-auto bg-[#E0E5EC] rounded-[20px] sm:rounded-[24px] px-3 xs:px-4 sm:px-6 py-2 sm:py-2.5 flex items-center justify-between shadow-neu-extruded gap-1.5 xs:gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-[#E0E5EC] shadow-neu-extruded-sm flex items-center justify-center p-1 group-hover:shadow-neu-extruded transition-all overflow-hidden flex-shrink-0">
              <img
                src="/app-icon.png"
                alt="SplitWise Logo"
          ...
          {!isAtWheel ? (
            <button
              type="button"
              onClick={() => onSwitchView && onSwitchView('wheel')}
              className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 xs:px-3.5 py-1.5 rounded-xl bg-[#E0E5EC] shadow-neu-extruded-sm hover:shadow-neu-extruded active:shadow-neu-pressed text-xs font-bold text-[#3D4852] transition-all cursor-pointer whitespace-nowrap flex-shrink-0"
        ...
        <div className="flex items-center gap-1.5 xs:gap-2 sm:gap-3 flex-shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 xs:gap-2 px-2.5 xs:px-3 py-1.5 rounded-xl bg-[#E0E5EC] shadow-neu-extruded-sm hover:shadow-neu-extruded active:shadow-neu-pressed text-xs sm:text-sm font-semibold text-[#3D4852] transition-all max-w-[100px] xs:max-w-[140px] sm:max-w-[200px] cursor-pointer"
            >
              <span className="truncate">{activeGroup?.name || 'Workspace'}</span>
              <ChevronDown
                className={`w-3.5 h-3.5 text-[#6B7280] flex-shrink-0 transition-transform duration-200 ${
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="w-8 h-8 xs:w-9 xs:h-9 rounded-xl bg-[#E0E5EC] shadow-neu-extruded-sm hover:shadow-neu-extruded active:shadow-neu-pressed flex items-center justify-center transition-all cursor-pointer flex-shrink-0"
          ...
          <button
            ...
            className="w-8 h-8 xs:w-9 xs:h-9 rounded-xl bg-[#E0E5EC] text-[#6B7280] hover:text-[#3D4852] shadow-neu-extruded-sm hover:shadow-neu-extruded active:shadow-neu-pressed flex items-center justify-center transition-all cursor-pointer flex-shrink-0"
