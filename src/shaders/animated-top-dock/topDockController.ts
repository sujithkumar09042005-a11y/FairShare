...
export function createTopDockController(
  ...
  const refreshItems = () => {
    ...
    if (items.length !== elements.length || items.some((item, i) => item.element !== elements[i])) {
  ...
  const canAnimate = () => !reducedQuery.matches && window.innerWidth > 480;
  ...
  const applyLayout = () => {
    ...
    for (const state of items) {
      ...
      state.element.style.transform = `translateY(${(value * options.drop).toFixed(2)}px) scale(${(1 + value * 0.04).toFixed(3)})`;
    }
  };
  ...
  const bar = root.closest<HTMLElement>(".atd-modern__bar") ?? root;
  const onPointerMove = (event: PointerEvent) => setTargets(event.clientX, event.clientY);
  const onWindowPointerMove = (event: PointerEvent) => {
    if (!pointerActive) return;
    const barRect = (root.closest<HTMLElement>(".atd-modern__bar") ?? root).getBoundingClientRect();
    const buffer = 45;
    const outside =
      event.clientX < barRect.left - buffer ||
      event.clientX > barRect.right + buffer ||
      event.clientY < barRect.top - buffer ||
      event.clientY > barRect.bottom + buffer;
    if (outside) reset();
  };
